...
template <typename Fr> struct BackingMemory {
  ...
  private:
    ...
    // Threshold for requesting huge/superpage backing (2 MiB pages).
    // At 2^24 (512 MiB per poly), 4 KiB pages cause ~131K TLB entries per poly.
    // Huge pages reduce TLB pressure by 512x, eliminating 5-10% overhead from
    // TLB misses during parallel_for traversals over multiple polynomials.
    static constexpr size_t HUGE_PAGE_THRESHOLD = 2 * 1024 * 1024; // 2 MiB
    ...
    static void allocate_aligned(BackingMemory& memory, size_t size)
    {
        ...
        if (byte_size >= ANON_MMAP_THRESHOLD) {
            // For large allocations, try superpage/huge page backing first.
            // This reduces TLB pressure significantly for polynomial-sized allocations.
            if (byte_size >= HUGE_PAGE_THRESHOLD) {
#ifdef __APPLE__
                // macOS: VM_FLAGS_SUPERPAGE_SIZE_2MB requests 2 MiB superpages.
                // Requires allocation size to be 2 MiB aligned.
                size_t aligned_size = (byte_size + (2 * 1024 * 1024 - 1)) & ~(size_t(2 * 1024 * 1024 - 1));
                void* addr = mmap(nullptr, aligned_size, PROT_READ | PROT_WRITE,
                                  MAP_ANONYMOUS | MAP_PRIVATE, VM_FLAGS_SUPERPAGE_SIZE_2MB, 0);
                if (addr != MAP_FAILED) {
                    memory.aligned_memory = std::shared_ptr<Fr[]>(
                        static_cast<Fr*>(addr), [aligned_size](Fr* p) { munmap(p, aligned_size); });
                    memory.raw_data = static_cast<Fr*>(addr);
                    memory.zero_initialized = true;
                    return;
                }
                // Superpage failed (common on low-memory systems), fall through to regular mmap
#elif defined(__linux__)
                // Linux: use regular mmap + madvise(MADV_HUGEPAGE) for transparent huge pages
                void* addr = mmap(nullptr, byte_size, PROT_READ | PROT_WRITE,
                                  MAP_ANONYMOUS | MAP_PRIVATE, -1, 0);
                if (addr != MAP_FAILED) {
                    madvise(addr, byte_size, MADV_HUGEPAGE);
                    memory.aligned_memory = std::shared_ptr<Fr[]>(
            ...
            if (addr != MAP_FAILED) {
                memory.aligned_memory = std::shared_ptr<Fr[]>(
                    static_cast<Fr*>(addr), [byte_size](Fr* p) {
                        // Hint kernel that pages are reclaimable before unmapping.
                        // MADV_FREE lets the kernel lazily reclaim pages without synchronously
                        // removing page table entries, reducing deallocation latency by 1-5ms
                        // per polynomial. The subsequent munmap is still needed for bookkeeping.
                        madvise(p, byte_size, MADV_FREE);
                        munmap(p, byte_size);
                    });
                memory.raw_data = static_cast<Fr*>(addr);
                memory.zero_initialized = true;
