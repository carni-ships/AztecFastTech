                                                                  ...
                                                                  std::vector<uint8_t>&& witness)
{
    ...
    // Use cached CommitmentKey: within one epoch, the same circuit type (same dyadic size)
    // is proved many times. The CK cache avoids SRS re-fetch + Metal GPU prewarm (~150-300ms)
    // on every proof after the first for each circuit size.
    size_t dyadic_size = builder.get_estimated_num_finalized_gates();
    // Round up to next power of 2 for dyadic sizing
    size_t dyadic_pow2 = 1;
    while (dyadic_pow2 < dyadic_size) dyadic_pow2 <<= 1;
    auto cached_ck = typename Flavor::CommitmentKey::get_or_create(dyadic_pow2);
    auto prover_instance = std::make_shared<ProverInstance_<Flavor>>(builder, cached_ck);

    auto final_time = std::chrono::high_resolution_clock::now();
    auto duration = std::chrono::duration_cast<std::chrono::milliseconds>(final_time - initial_time);
    info("CircuitProve: Proving key computed in ", duration.count(), " ms (CK cached)");
