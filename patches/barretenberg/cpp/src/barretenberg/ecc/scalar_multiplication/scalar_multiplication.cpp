...
{
    // Fast path: cache results for repeated calls with the same num_points.
    // During epoch proving, the same circuit sizes (and therefore MSM sizes) repeat
    // dozens of times. The cache avoids recomputing the cost model each time.
    static thread_local std::pair<size_t, uint32_t> cached = { 0, 0 };
    if (cached.first == num_points && num_points > 0) {
        return cached.second;
    }

    // Cost model: total_cost = num_rounds * (num_points + num_buckets * bucket_cost)
    // The bucket cost includes a cache-miss penalty for large bucket arrays that
    // exceed the L3 cache. On Apple M3 Pro (36 MB L3), each Jacobian point is 96 bytes:
    //   2^15 buckets = 3 MB (L3-resident, fast accumulation)
    //   2^18 buckets = 24 MB (partial L3 hit)
    //   2^20 buckets = 96 MB (mostly L3 misses, ~3x slower accumulation)
    auto compute_cost = [&](uint32_t bits) {
        ...
        // Cache-aware bucket cost: base cost + penalty for L3 overflow
        // 96 bytes per Jacobian point, ~36 MB L3 → ~375K buckets fit in L3
        constexpr size_t L3_BUCKET_CAPACITY = 375000;
        size_t bucket_cost = BUCKET_ACCUMULATION_COST;
        if (buckets > L3_BUCKET_CAPACITY) {
            bucket_cost += 2; // ~40% penalty for cache-miss-dominated accumulation
        }
        return rounds * (num_points + buckets * bucket_cost);
    };
