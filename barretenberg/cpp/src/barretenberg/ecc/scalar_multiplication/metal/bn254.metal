...
kernel void msm_reduce_gathered(
    ...
    // Threshold is passed dynamically from host to match the CSM kernel's threshold.
...
// BN254 scalar field order r (32-bit limbs, little-endian)
constant uint FR_P[8] = {
    0xf0000001, 0x43e1f593, 0x79b97091, 0x2833e848,
    0x8181585d, 0xb85045b6, 0xe131a029, 0x30644e72
};

// Montgomery inverse for scalar field: -r^{-1} mod 2^32
constant uint FR_INV = 0xefffffffu;

// Convert scalar from Montgomery form to standard form.
// Computes a * R^{-1} mod r using 8×32-bit CIOS reduction (b = 1).
// Skips the a*b multiply inner loop since b = {1, 0, 0, 0, 0, 0, 0, 0}.
void fr_from_montgomery(thread ulong* kr) {
    uint s[8] = {
        uint(kr[0] & 0xFFFFFFFF), uint(kr[0] >> 32),
        uint(kr[1] & 0xFFFFFFFF), uint(kr[1] >> 32),
        uint(kr[2] & 0xFFFFFFFF), uint(kr[2] >> 32),
        uint(kr[3] & 0xFFFFFFFF), uint(kr[3] >> 32)
    };
    ...
    #pragma clang loop unroll(full)
    for (int i = 0; i < 8; i++) {
        // Add s[i] to t[0] with carry propagation (multiply by 1)
        ulong carry = ulong(t[0]) + ulong(s[i]);
        t[0] = uint(carry & 0xFFFFFFFF);
        carry >>= 32;
        #pragma clang loop unroll(full)
        for (int j = 1; j < 9; j++) {
            carry += ulong(t[j]);
    ...
    // Conditional subtraction: if result >= r, subtract r
    bool need_sub = (t[8] != 0);
    if (!need_sub) {
        for (int i = 7; i >= 0; i--) {
            if (t[i] > FR_P[i]) { need_sub = true; break; }
            if (t[i] < FR_P[i]) break;
        }
    }
    if (need_sub) {
        ...
        #pragma clang loop unroll(full)
        for (int i = 0; i < 8; i++) {
            borrow += long(t[i]) - long(FR_P[i]);
            t[i] = uint(borrow & 0xFFFFFFFF);
            borrow >>= 32;
        }
    }

    // Pack back to 4×ulong
    kr[0] = ulong(t[0]) | (ulong(t[1]) << 32);
    kr[1] = ulong(t[2]) | (ulong(t[3]) << 32);
    kr[2] = ulong(t[4]) | (ulong(t[5]) << 32);
    kr[3] = ulong(t[6]) | (ulong(t[7]) << 32);
}
...
// When from_montgomery is non-zero, scalars are in Montgomery form and
// will be converted to standard form on-GPU before GLV decomposition.
kernel void glv_decompose(
    ...
    constant uint& from_montgomery [[buffer(6)]],
    ...
    // Convert from Montgomery form if needed (fused into GLV, no CPU conversion)
    if (from_montgomery) {
        fr_from_montgomery(kr);
    }
...
kernel void glv_apply_neg_flags(
    device const PointAffine* cache [[buffer(0)]],  // cache_stride*2 entries (original + endo)
    device PointAffine* points [[buffer(1)]],       // output: 2n entries
    const device uchar* neg1_flags [[buffer(2)]],
    const device uchar* neg2_flags [[buffer(3)]],
    constant uint& n [[buffer(4)]],
    constant uint& cache_stride [[buffer(5)]],      // stride in endo cache (>= cache_offset + n)
    constant uint& cache_offset [[buffer(6)]],      // offset into cache for this MSM's SRS region
    uint gid [[thread_position_in_grid]]
) {
    if (gid >= n) return;

    PointAffine p = cache[cache_offset + gid];
    if (neg1_flags[gid]) {
        p.y = fp_neg(p.y);
    }
    points[gid] = p;

    PointAffine endo = cache[cache_stride + cache_offset + gid];
    if (neg2_flags[gid]) {
