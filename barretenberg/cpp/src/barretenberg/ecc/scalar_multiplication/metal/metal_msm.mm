...
#pragma clang diagnostic pop
    ...
    NSString* load_shader_source()
    {
        ...
        {
            NSString* path = @METAL_SHADER_PATH;
            NSString* source = [NSString stringWithContentsOfFile:path encoding:NSUTF8StringEncoding error:nil];
            if (source) {
                return source;
            }
    ...
    void ensure_buffers(size_t n, size_t n_buckets, size_t n_segments, size_t n_windows)
    {
        ...
        size_t points_bytes = sizeof(GpuPointAffine) * np2;
        size_t sorted_bytes = sizeof(uint32_t) * np2 * nw;
        size_t max_buf = [device maxBufferLength];
        if (points_bytes > max_buf || sorted_bytes > max_buf) {
            fprintf(stderr, "[GPU] FATAL: buffer exceeds maxBufferLength=%zuMB (points=%zuMB, sorted=%zuMB)\n",
                    max_buf / (1024*1024), points_bytes / (1024*1024), sorted_bytes / (1024*1024));
        }
        points_buffer = [device newBufferWithLength:points_bytes
                                           options:MTLResourceStorageModeShared];
        sorted_indices_buffer = [device newBufferWithLength:sorted_bytes
                                                   options:MTLResourceStorageModeShared];
        ...
        // Validate all critical buffer allocations succeeded
        if (!points_buffer || !sorted_indices_buffer || !all_offsets_buffer || !all_counts_buffer ||
            !buckets_buffer || !segment_results_buffer || !window_results_buffer ||
            !count_sorted_map_buffer || !large_bucket_ids_buffer || !scatter_pos_buffer ||
            !n_large_buckets_out_buffer || !imbalance_flag_buffer) {
            fprintf(stderr, "[GPU] FATAL: Metal buffer allocation failed (n=%zu, total ~%zuMB)\n",
                    np, (sizeof(GpuPointAffine) * np2 + sizeof(uint32_t) * np2 * nw) / (1024*1024));
            // Reset to force re-allocation next time
            max_points = 0;
            return;
        }
  ...
  public:
    ...
    void prewarm(size_t n, const curve::BN254::AffineElement* srs_points = nullptr)
    {
        ...
        // Warm the Metal command queue, pipeline states, and endo cache.
        // Without this, the first real MSM pays ~150ms of Metal runtime initialization
        // plus ~100ms for endo precompute (applying beta endomorphism to all SRS points).
        {
            id<MTLCommandBuffer> cb = [command_queue commandBuffer];

            // Trigger pipeline state JIT compilation with a trivial glv_decompose dispatch.
            ...
            [enc setBytes:&zero length:sizeof(uint32_t) atIndex:6];
            [enc dispatchThreads:MTLSizeMake(1, 1, 1) threadsPerThreadgroup:MTLSizeMake(1, 1, 1)];
            [enc endEncoding];

            // Precompute endo cache (beta * SRS points) if SRS was provided.
            // This moves ~100ms of endo precompute from the first MSM to prewarm time.
            if (srs_points && n > 0 && srs_buffer) {
                ...
                id<MTLComputeCommandEncoder> enc_endo = [cb computeCommandEncoder];
                [enc_endo setComputePipelineState:glv_precompute_cache_fn];
                [enc_endo setBuffer:srs_buffer offset:0 atIndex:0];
                [enc_endo setBuffer:endo_cache_buffer offset:0 atIndex:1];
                [enc_endo setBytes:&n_u32 length:sizeof(uint32_t) atIndex:2];
                MTLSize grid = MTLSizeMake(n, 1, 1);
                MTLSize tg = MTLSizeMake(std::min(size_t(256), n), 1, 1);
                [enc_endo dispatchThreads:grid threadsPerThreadgroup:tg];
                [enc_endo endEncoding];
                cached_endo_srs_ptr = srs_points;
                cached_endo_count = n;
            }

            [cb commit];
            // Don't wait — Metal command buffers execute in queue order, so the first real MSM
            // will naturally wait for this dispatch to finish.
        }
    }
     ...
     */
    bool run_msm(const curve::BN254::AffineElement* srs_base,
                 size_t srs_start_offset,
                 ...
                 bool from_montgomery = false)
    {
        @autoreleasepool {
            ...
            // SRS point caching: key on srs_base pointer, cache enough points for offset + n.
            // Using the base pointer (not offset) ensures the cache is stable across different
            // start_index values, avoiding unnecessary re-copies.
            size_t srs_needed = srs_start_offset + n;
            if (srs_base != cached_srs_ptr || srs_needed > cached_srs_count) {
                size_t copy_count = (srs_base == cached_srs_ptr) ? std::max(srs_needed, cached_srs_count) : srs_needed;
                ...
                std::memcpy(static_cast<GpuPointAffine*>([srs_buffer contents]),
                            srs_base, sizeof(GpuPointAffine) * copy_count);
                cached_srs_ptr = srs_base;
                cached_srs_count = copy_count;
            }

            // Zero-copy scalar sharing: wrap the polynomial's mmap'd memory as a Metal buffer.
            // On Apple Silicon unified memory, this lets the GPU read directly from the polynomial
            // without a 128 MiB memcpy per commit. Requires page-aligned pointer (4096 bytes).
            // Falls back to memcpy if pointer is not page-aligned (e.g. non-zero start_index).
            id<MTLBuffer> zero_copy_scalars = nil;
            size_t scalar_bytes = sizeof(curve::BN254::ScalarField) * n;
            uintptr_t ptr_addr = reinterpret_cast<uintptr_t>(scalars);
            if ((ptr_addr & 4095) == 0) {
                // Page-aligned: try zero-copy
                size_t aligned_bytes = (scalar_bytes + 4095) & ~size_t(4095);
                zero_copy_scalars = [device newBufferWithBytesNoCopy:const_cast<curve::BN254::ScalarField*>(scalars)
                                                             length:aligned_bytes
                                                            options:MTLResourceStorageModeShared
                                                        deallocator:nil];
            }
            if (!zero_copy_scalars) {
                // Fallback: copy to pre-allocated GPU buffer
                auto* gpu_scalars_dst = static_cast<curve::BN254::ScalarField*>([scalars_buffer contents]);
                std::memcpy(gpu_scalars_dst, scalars, scalar_bytes);
            }
            ...
            // compute encoders for data dependencies.
            {
                ...
                // Use zero-copy buffer if available, fall back to pre-allocated scalars_buffer
                id<MTLBuffer> active_scalars = zero_copy_scalars ? zero_copy_scalars : scalars_buffer;
                id<MTLComputeCommandEncoder> enc_dec = [cb_prep computeCommandEncoder];
                [enc_dec setComputePipelineState:glv_decompose_fn];
                uint32_t mont_flag = from_montgomery ? 1u : 0u;
                [enc_dec setBuffer:active_scalars offset:0 atIndex:0];
                ...
                [enc_dec setBytes:&mont_flag length:sizeof(uint32_t) atIndex:6];
                [enc_dec dispatchThreads:grid_n threadsPerThreadgroup:tg_n];
                [enc_dec endEncoding];

                // GLV Kernel 2: Endomorphism (cached or full).
                // Cache layout: [original[0..cached_endo_count-1] | endo[0..cached_endo_count-1]].
                // Keyed on srs_base (not offset pointer) so the cache is stable across different
                // start_index values. Valid when srs_base matches and offset+n <= cached count.
                bool endo_cached = (srs_base == cached_endo_srs_ptr && srs_needed <= cached_endo_count);
                if (!endo_cached) {
                    // First time for this SRS or needs more points than cached
                    size_t endo_count = (srs_base == cached_endo_srs_ptr) ? std::max(srs_needed, cached_endo_count) : srs_needed;
                    if (!endo_cache_buffer || [endo_cache_buffer length] < sizeof(GpuPointAffine) * 2 * endo_count) {
                        endo_cache_buffer = [device newBufferWithLength:sizeof(GpuPointAffine) * 2 * endo_count
                                                               options:MTLResourceStorageModeShared];
                    }
                    uint32_t endo_n_u32 = static_cast<uint32_t>(endo_count);
                    MTLSize grid_endo = MTLSizeMake(endo_count, 1, 1);
                    MTLSize tg_endo = MTLSizeMake(std::min(size_t(256), endo_count), 1, 1);
                    ...
                    [enc_precomp setBytes:&endo_n_u32 length:sizeof(uint32_t) atIndex:2];
                    [enc_precomp dispatchThreads:grid_endo threadsPerThreadgroup:tg_endo];
                    [enc_precomp endEncoding];
                    cached_endo_srs_ptr = srs_base;
                    cached_endo_count = endo_count;
                }
                // Apply neg flags from cache (always needed — neg flags change per MSM).
                // Pass cache_stride (= cached_endo_count) for the endo half offset,
                // and cache_offset (= srs_start_offset) so the shader reads the right SRS region.
                uint32_t cache_stride_u32 = static_cast<uint32_t>(cached_endo_count);
                uint32_t cache_offset_u32 = static_cast<uint32_t>(srs_start_offset);
                ...
                [enc_neg setBytes:&cache_stride_u32 length:sizeof(uint32_t) atIndex:5];
                [enc_neg setBytes:&cache_offset_u32 length:sizeof(uint32_t) atIndex:6];
                ...
                // Dynamic large-bucket threshold: must match between CSM and reduce_gathered.
                // Scale with n to avoid classifying too many buckets as "large".
            ...
            uint32_t n_large = *static_cast<uint32_t*>([n_large_buckets_out_buffer contents]);
            if (imbalance_val != 0) {
                // Imbalance flag set by mc > n2/4 (bucket concentration) or buffer overflow.
                // The large-bucket reduce kernel handles any bucket count correctly, so
                // concentration alone is not fatal. Only fall back on actual buffer overflow.
                uint32_t large_buf_cap = static_cast<uint32_t>(n_buckets * n_windows);
                if (n_large > large_buf_cap) {
                    return false;
                }
            }
                                            ...
                                            bool skip_imbalance_check)
{
    ...
    // Pass SRS base pointer + start offset so the endo cache can be keyed on the stable
    // base pointer (avoids cache invalidation when start_index alternates between 0 and 1).
    curve::BN254::Element result;
    bool ok = ctx.run_msm(points.data(), start, &scalars[start], num_points, result, skip_imbalance_check,
                                                ...
                                                bool skip_imbalance_check)
{
    ...
    curve::BN254::Element result;
    bool ok = ctx.run_msm(points.data(), /*srs_start_offset=*/0, scalars.data(), num_points, result, skip_imbalance_check,
                          /*from_montgomery=*/true);
