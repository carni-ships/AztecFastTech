...
template <typename Fr> class Polynomial {
  public:
    ...
    static Polynomial random(size_t size, size_t virtual_size, size_t start_index)
    {
        ...
        // Bulk random generation: fill the entire polynomial buffer with random bytes in one call
        // to avoid lock contention from multiple threads hitting arc4random_buf's global lock.
        // Then parallel-correct elements that exceed the field modulus.
        // Skip Montgomery conversion: store raw bytes directly as Montgomery representation.
        // Since R^{-1} is a bijection on Z_p, a uniform value in [0,p) stored as Montgomery form
        // represents a uniform field element — exactly what we need for random polynomials.
        FF* data = p.coefficients_.data();
        static_assert(sizeof(FF) == 32, "Expected 32-byte field elements for bulk random fill");

        // Phase 1: Bulk fill with random bytes — single-threaded to avoid CSPRNG lock contention.
        // arc4random_buf handles large buffers efficiently in one call (~2-3 GB/s on Apple Silicon).
        {
            auto* raw = reinterpret_cast<uint8_t*>(data);
            size_t total_bytes = size * sizeof(FF);
#if defined(__APPLE__) && !TARGET_OS_IPHONE && !TARGET_IPHONE_SIMULATOR
            arc4random_buf(raw, total_bytes);
#else
            // Fallback: use the thread-local RNG for non-Apple platforms
        ...
        // Phase 2: Parallel correction — fix elements >= modulus with rejection sampling.
        // BN254 Fr modulus is ~2^253.97, so ~12.3% of elements need correction.
        parallel_for_heuristic(
            size,
            [&](size_t start, size_t end, [[maybe_unused]] size_t chunk_index) {
                auto& rng = numeric::get_randomness();
                for (size_t i = start; i < end; i++) {
                    // Interpret the 4 limbs as a uint256 and check against modulus
                    uint256_t raw(data[i].data[0], data[i].data[1], data[i].data[2], data[i].data[3]);
                    if (!(raw < FF::modulus)) {
                        // Element exceeds modulus — regenerate with rejection sampling
                        do {
                            raw = rng.get_random_uint256();
                        } while (!(raw < FF::modulus));
                        data[i] = FF(raw.data[0], raw.data[1], raw.data[2], raw.data[3]);
                    }
                }
            },
