...
template <IsUltraOrMegaHonk Flavor_> class ProverInstance_ {
  ...
  public:
        ...
        : commitment_key(commitment_key)
    {
        ...
        // Sparse optimization: only fill [0, active_end) and [dyadic-3, dyadic) with random values,
        // leaving the gap zeroed. This lets the commit function use the sparse MSM split, cutting
        // GPU MSM work by ~46%. Security is preserved because the polynomial has ~active_end random
        // evaluations, far exceeding the ~22 Gemini opening points.
        std::future<Polynomial> masking_poly_future;
        if constexpr (Flavor::HasZK) {
            masking_poly_future = std::async(
                std::launch::async, [dyadic_sz = metadata.dyadic_size, active_end = final_active_wire_idx + 1] {
                    // Allocate full-size zeroed polynomial, then fill active region + tail with random
                    Polynomial poly(dyadic_sz, dyadic_sz);

                    // Fill [0, active_end) with random field elements using bulk random generation
                    FF* data = poly.data();
                    {
                        auto* raw = reinterpret_cast<uint8_t*>(data);
                        size_t active_bytes = active_end * sizeof(FF);
#if defined(__APPLE__) && !TARGET_OS_IPHONE && !TARGET_IPHONE_SIMULATOR
                        arc4random_buf(raw, active_bytes);
#else
                        auto& rng = numeric::get_randomness();
                        for (size_t i = 0; i < active_end; i++) {
                            uint256_t val = rng.get_random_uint256();
                            data[i] = FF(val.data[0], val.data[1], val.data[2], val.data[3]);
                        }
#endif
                    }
                    // Correct elements >= modulus (BN254 Fr ~12.3% rejection rate)
                    {
                        auto& rng = numeric::get_randomness();
                        for (size_t i = 0; i < active_end; i++) {
                            uint256_t raw_val(data[i].data[0], data[i].data[1], data[i].data[2], data[i].data[3]);
                            if (!(raw_val < FF::modulus)) {
                                do {
                                    raw_val = rng.get_random_uint256();
                                } while (!(raw_val < FF::modulus));
                                data[i] = FF(raw_val.data[0], raw_val.data[1], raw_val.data[2], raw_val.data[3]);
                            }
                        }
                    }
                    // Add 3 random masking values at [dyadic_size-3, dyadic_size)
                    auto& engine = numeric::get_randomness();
                    for (size_t i = dyadic_sz - NUM_MASKED_ROWS; i < dyadic_sz; i++) {
        ...
        // Launch lookup table and read count construction asynchronously.
        // These access circuit.lookup_tables (disjoint from circuit.blocks used by Trace::populate),
        // and write to separate polynomials, so concurrent execution is safe.
        auto lookup_table_future = std::async(std::launch::async, [&] {
        ...
        auto lookup_read_counts_future = std::async(std::launch::async, [&] {
            BB_BENCH_NAME("constructing lookup read counts");
            construct_lookup_read_counts<Flavor>(
                polynomials.lookup_read_counts, polynomials.lookup_read_tags, circuit);
        });
        ...
        // Wait for async lookup construction to complete
        lookup_table_future.get();
        lookup_read_counts_future.get();
